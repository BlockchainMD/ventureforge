 ...
 *
 * Lives in `db` rather than `ingestion` so that seeding a fresh database is
 * self-sufficient: without it, `pnpm setup` would create the schema, seed the
 * comps, and then silently skip every fixture parcel because no `Source` rows
 * existed to attach them to.
 *
